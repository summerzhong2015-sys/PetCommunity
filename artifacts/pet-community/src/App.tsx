...
function Messages({ notify }: { notify: (n: Notice) => void }) {
  ...
  async function sendRequest() {
    ...
    // A first hello is the one message a neighbour cannot choose not to read.
    const moderated = await review(opener);
    if (moderated.level === 'block') { notify({ tone: 'error', text: moderated.reason }); return; }
